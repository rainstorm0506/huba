...
class SplitWord
{
	...
	function __construct ($source_charset= 'utf-8' , $target_charset= 'utf-8' , $load_all= TRUE , $source= '')
	{
		...
		$this->yiiExtSP = chr(0xFF).chr(0xFE);
		
		$this -> SetSource ($source , $source_charset , $target_charset) ;
		$this -> isLoadAll= $load_all ;
		if (file_exists ($this->currentDir. '/'. $this -> mainDicFile)) $this -> isUnpacked= TRUE ;
	...
	{
		$this -> addonDicFile= $this->currentDir. '/'. $this -> addonDicFile ;
		$this -> mainDicFile= $this->currentDir. '/'. $this -> mainDicFile ;
		$this -> mainDicFileZip= $this->currentDir. '/'. $this -> mainDicFileZip ;
...
else
			{
...
else
				{
					...
					if ($cn== 0x300A)
					{
						...
						while (TRUE)
						{
							if (! isset ($this -> sourceString[$i+ $n])&& ! isset ($this -> sourceString[$i+ $n+ 1])) break ;
	...
	public function GetFinallyResultArray($noun = true)
	{
		$res = array();
	...
	{
		return $this->GetWordPropertys($word) == 'n';
	}
	
	function GetWordPropertys($word)
	{
		if (strlen($word)<4)
			return 's';
		...
		$infos= $this -> GetWordInfos($word) ;
		//return isset ($infos[1]) ? "{$infos[1]}{$infos[0]}" : 's';
		return isset ($infos[1]) ? $infos[1] : 's';
	}
	...
	{
		if (! ini_set ('memory_limit' , '128M')) exit ('设置内存错误，请到dede官网下载解压版的base_dic_full.dic!') ;
		require_once ($this->currentDir. '/zip.class.php') ;
		$zip= new zip () ;
		// echo $targetfile;
		$unpackagefile= array_keys ($zip -> Extract ($targetfile , $this->currentDir. '/')) ;
		// exit();
		$this -> MakeDict ($this->currentDir. '/'. $unpackagefile[0]) ;
		unlink ($this->currentDir. '/'. $unpackagefile[0]) ;
		return true ;
	}
}